...
### 7.6b Agent 1.4 — Instruction Planner (only when `--instructions` is given)
A creator's brief is a PARAGRAPH, however long, and dumping it verbatim into both prompts wastes it: it never influenced the research **queries** at all, so the pipeline never went and *found* the material the brief asked for. The planner decomposes it into a routed plan (`research_focus`, `research_queries`, `script_directions`, `outline`, `avoid`, `terminology`), written to `instruction_plan.json` for inspection. An item may be routed to BOTH sides — research verifies a term, the script then uses it.

- **Asks are enumerated, not split on punctuation.** A single sentence routinely carries five or six separate asks, so counting sentences under-counts badly: on a real 1603-char brief, sentence-splitting found **6** where LLM ask-enumeration found **23**. That mattered twice over, because a long multi-ask sentence also trivially clears a fractional word-overlap coverage bar even when half its asks were dropped — the wrong unit does not merely under-report, it *launders* a failure into a pass. `_enumerate_asks` makes one cheap `temperature=0.0` call; sentence-splitting survives only as a documented fallback.
- **Coverage is repaired, not just reported.** `uncovered()` flags any ask sharing less than `_COVERED_FRACTION` of its distinctive words with the plan (a FRACTION, not a count — a flat "share N words" bar is trivial for a 30-word ask and impossible for a 5-word one). Up to `_MAX_PASSES` re-plan **only** the missed asks. The repair RE-PLANS rather than pasting the raw sentence in, so the creator's raw hand-written wording never leaks into the script prompt. Residual misses are logged loudly as `instruction_plan_incomplete`.
- **The per-bucket cap scales with the ask count** (`_item_cap`), because a fixed cap is a silent truncation waiting to happen — and a truncated bucket is indistinguishable from a planner that ignored the ask.
- Coverage is not the same as **fidelity**: whether the resulting script keeps the creator's *figures* unchanged is enforced separately, by the Judge (see 9.3).

Gated by `INSTRUCTION_PLANNER_ENABLED`; any failure degrades to the verbatim steer, so the run continues.
