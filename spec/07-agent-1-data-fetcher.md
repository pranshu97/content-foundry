...
### 7.1 Purpose
Ground the entire pipeline in **reality**. Agent 1 pulls fresh labor-market signals from external sources, normalizes them, then **deterministically** distills them into a structured, citation-ready `DataBrief` — **no LLM call**. Because the sources return structured numbers (salaries, posting counts, layoff events), facts are extracted by code, which is free, fast, and guarantees zero hallucination. Everything downstream cites this brief.
