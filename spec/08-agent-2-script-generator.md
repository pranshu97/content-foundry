...
- **No repeated scenes (prompt + hard gate):** a top prompt rule forbids reusing a sentence or restating a fact across scenes (each figure is stated at most once); the Judge independently REVISEs any script whose scenes are near-verbatim duplicates (`MAX_SCENE_SIMILARITY`), naming the offending pairs so the rewrite targets exactly them.
