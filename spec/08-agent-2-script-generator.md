...
- **Revisions EDIT, not regenerate:** on a revision the Generator is handed its own previous draft (the full script JSON) and told to change ONLY what the feedback flags, keeping every already-good scene — and especially the closing like/subscribe nudge + sign-off — verbatim. This stops the loop oscillating (e.g. fixing wit but dropping the ending). A Judge-forced template shift is the one case that regenerates fresh, since the structure itself must change.
...
- **Sources are on-screen only (never spoken):** every scene that states a statistic surfaces the referenced fact's exact source in `on_screen_text` (`… · Source: Adzuna`), stamped **deterministically** in a post-pass (`_stamp_sources`) so a stat can never appear un-sourced. Web-search facts (which have no fixed label) show the site's **domain** taken from the citation URL (`Source: msoe.edu`) instead of a generic "Source: Search". Stamping is triggered by a *stated number*, so a script with **zero figures shows no citations at all** — the generator is therefore required to speak at least two specific numbers drawn from the facts. The narration must never *say* the source out loud; the prompt forbids it and `_clean_narration` strips any spoken/bracketed source that leaks through.
...
- **Voice, wit & ending:** wit is a **core requirement**, not a garnish — the prompt's two top rules demand a genuinely funny script (the Judge enforces a `WITTINESS_MIN` floor) and a strong close: the final scene must land a payoff line plus **both** a like/subscribe nudge and a warm sign-off (the Judge's deterministic `ENDING_MIN` floor requires both). Every fact stays accurate.
- **No repeated scenes (prompt + code guarantee + hard gate):** a top prompt rule forbids reusing a sentence or restating a fact across scenes (each figure at most once); the Generator then **deterministically DROPS any scene that near-duplicates an earlier one** (`dedupe_scene_indices`, the same `MAX_SCENE_SIMILARITY` 3-gram measure) *before* grading, so recycled/padded copies never reach the Judge — the length gate re-expands if trimming left the draft short, forcing NEW content over repeats. The Judge still independently REVISEs any near-verbatim duplicates that somehow survive, naming the offending pairs. (Detection alone was not enough: a weak local model under a high `SCENES_PER_VIDEO` target kept padding by copying, so the drop is a code guarantee, not a plea.)
- **B-roll shots (ordered per beat):** `b_roll_keywords` is an ordered list where each entry is a concrete, filmable shot description (a balanced 3–4 words — a subject plus its action or setting) for one beat of the scene, matched to what is being said at that moment; Agent 5 fetches a separate clip per beat, trimming each to a stock-searchable query via `_search_terms`.
