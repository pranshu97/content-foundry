...
- **Creator's own figures (hard gate):** a quantity written into `--instructions` must come back unchanged. A brief saying model code is "ten percent of the job" produced a script saying "five percent" while keeping the matching "ninety percent" — contradicting both the instruction and itself, and the thumbnail then faithfully rendered the wrong number. Instruction *coverage* cannot see this: the ask was covered and the topic discussed, only the quantity was rewritten. `specified_figures_report` normalises digits, number-words and units (`percent`, `%`, `ms`, `x`/`times`, `dollars`) and flags a figure **only when the script uses the SAME UNIT with a DIFFERENT value** — a figure the script never raises is left to the coverage machinery, extra researched figures are never penalised, and bare years/decades are excluded ("the 1970s" → "the seventies" is a fair paraphrase). The note is placed FIRST in the revision instructions: everything else is craft, this one is factually wrong.
