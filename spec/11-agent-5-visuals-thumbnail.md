...
- **Thumbnail:** base image (generated from `Script.thumbnail_concept`) + bold overlay text via `Pillow`, sized to `effective_thumbnail_size` (vertical 9:16 for a Short via `SHORTS_THUMBNAIL_SIZE`, else `THUMBNAIL_SIZE`). The image prompt is written by the **Thumbnail Director (Agent 5.6, `THUMBNAIL_DIRECTOR_ENABLED`)**: an LLM turns the `thumbnail_concept` / title / niche into a rich, high-CTR image-generation prompt that ends with a hard no-text clause (so the model stops baking in gibberish lettering); the exact prompt is saved to the editable `assets/thumbnail_prompt.txt`, an explicit/saved prompt bypasses the director, and it degrades to a built-in template when off/unavailable. When `THUMBNAIL_USE_AVATAR` is on and `assets/avatar.png` exists, your face is composited in: an opaque source is background-removed with **`rembg`** (cached as `<name>.cutout.png`) and scaled by `THUMBNAIL_AVATAR_SCALE`. An empty `thumbnail_text` falls back to a shortened form of the title rather than dumping the whole title. The overlay **text** itself is read from `Script.thumbnail_text` (fallback: the first `title_options`, then `thumbnail_concept`) in `script.json` — NOT `visuals.json` (whose `thumbnail_text` is only an output record), so to change it edit `script.json` and re-run `content-foundry thumbnail`.
